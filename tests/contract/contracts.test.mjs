...
test('CLI verifies a canonical assurance plan without loading repository configuration', async () => {
  const directory = mkdtempSync(join(tmpdir(), 'l9-assurance-plan-verify-'));
  try {
    const output = join(directory, 'plan.json');
    ...
    let stdout = '';
    const verifyCode = await runCli(['verify-plan', '--plan', output], {
    ...
    assert.match(report.planId, /^plan_[a-f0-9]{40}$/);
