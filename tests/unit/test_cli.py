...
from l9_assurance.cli import CLI_COMMANDS, parse_argv, run_cli
...
def test_duplicate_flag_rejected() -> None:
    errors: list[str] = []
    code = run_cli(["verify", "--decision", "a", "--decision", "b"], stdout=lambda _: None, stderr=errors.append)
    assert code == 40
...
def test_plan_and_verify_plan_commands(tmp_path: Path, subject: dict) -> None:
    ...
    assert run_cli(["plan", "--profile", "l9.pull-request@1", "--subject", str(subject_path), "--output", str(plan_path)], stdout=lambda _: None, stderr=lambda _: None) == 0
    assert run_cli(["verify-plan", "--plan", str(plan_path)], stdout=lambda _: None, stderr=lambda _: None) == 0
