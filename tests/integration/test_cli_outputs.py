...
def _trusted_protocol(tmp_path: Path, repo: Path) -> Path:
    ...
    preimage = {
        key: manifest[key]
        for key in ("schema", "schemaVersion", "assuranceVersion", "canonicalization", "files")
    }
    ...
    assert (
        run_cli(
            [
                ...
                *common,
                "--subject",
                str(repo / "fixtures/valid/subject.json"),
                "--input",
                str(observations),
                "--received-at",
...
def test_simulate_is_non_authoritative(tmp_path: Path, engine, subject, valid_observations) -> None:
    ...
    for item in report["accepted"]:
        (evidence / f"{item['envelope']['evidenceId']}.json").write_text(
            json.dumps(item["envelope"])
        )
    ...
    assert code == 0
    assert (
        json.loads((output / "decision.json").read_text())["extensions"]["l9.assurance.simulation"][
            "authoritative"
        ]
        is False
    )
