...
def test_failure_and_waiver(engine, subject, valid_observations) -> None:
    ...
    observations = [item for item in valid_observations if item["check"]["id"] != "l9.lint"] + [
        failed
    ]
    ...
    conditional = engine.evaluate(
        subject, admission["accepted"], evaluation_time="2026-07-21T00:00:03Z", waivers=[waiver]
    )
...
def test_mandatory_finding_fails(engine, subject, valid_observations) -> None:
    ...
    observations = [
        item for item in valid_observations if item["check"]["id"] != "l9.mandatory-findings"
    ] + [finding]
    admission = engine.admit(subject, observations, received_at="2026-07-21T00:00:02Z")
    decision = engine.evaluate(
        subject, admission["accepted"], evaluation_time="2026-07-21T00:00:03Z"
