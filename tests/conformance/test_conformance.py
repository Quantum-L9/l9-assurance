...
def test_producer_conformance_passes(trusted_config, subject, valid_observations) -> None:
    report = run_producer_conformance(
        ...
        subject=subject,
        producer_registry=trusted_config["producerRegistry"],
        check_registry=trusted_config["checkRegistry"],
...
def test_consumer_conformance_passes(engine, subject, valid_observations) -> None:
    ...
    decision = engine.evaluate(
        subject, admission["accepted"], evaluation_time="2026-07-21T00:00:03Z"
    )
    report = run_consumer_conformance(
        consumer_id="l9-ci-core",
        canonical_decision=decision,
        transported_decision_text=canonical_json(decision) + "\n",
        published_verdict=decision["verdict"],
        published_summary=render_decision_summary(decision),
    )
    assert report["passed"]
...
def test_consumer_byte_mutation_fails(engine, subject, valid_observations) -> None:
    ...
    report = run_consumer_conformance(
        consumer_id="l9-ci-core",
        canonical_decision=decision,
        transported_decision_text=json.dumps(decision, indent=2),
        published_verdict=decision["verdict"],
        published_summary=render_decision_summary(decision),
    )
