...
FIXTURES = (
    (ROOT / "fixtures" / "compatibility" / "producer-registry.trusted.json", Path("fixtures/compatibility/producer-registry.trusted.json")),
    (ROOT / "fixtures" / "compatibility" / "check-registry.json", Path("fixtures/compatibility/check-registry.json")),
    (ROOT / "fixtures" / "conformance" / "canonicalization-v1.json", Path("fixtures/conformance/canonicalization-v1.json")),
)
...
def expected_files() -> dict[str, bytes]:
    ...
    for source_root, destination_root in SOURCE_GROUPS:
        for source in sorted(path for path in source_root.rglob("*") if path.is_file()):
            output[(destination_root / source.relative_to(source_root)).as_posix()] = source.read_bytes()
...
def manifest(files: dict[str, bytes]) -> dict:
    entries = [{"path": path, "digest": hashlib.sha256(content).hexdigest()} for path, content in sorted(files.items())]
    preimage = {
    ...
    digest = hashlib.sha256(json.dumps(preimage, separators=(",", ":"), ensure_ascii=False).encode()).hexdigest()
    return {
        **preimage,
...
def write_bundle(files: dict[str, bytes]) -> None:
    ...
    (TARGET / "manifest.json").write_text(json.dumps(manifest(files), indent=2) + "\n", encoding="utf-8")
...
def check_bundle(files: dict[str, bytes]) -> list[str]:
    ...
    actual_manifest = (TARGET / "manifest.json").read_text(encoding="utf-8") if (TARGET / "manifest.json").is_file() else ""
    if actual_manifest != expected_manifest:
        failures.append("manifest.json")
    visible = {path.relative_to(TARGET).as_posix() for path in TARGET.rglob("*") if path.is_file() and path.name != "__init__.py"}
