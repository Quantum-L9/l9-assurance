...
EVALUATOR_FORBIDDEN_IMPORTS = {"os", "pathlib", "socket", "subprocess", "urllib", "http", "requests", "httpx"}
...
def main() -> int:
    ...
    for path in SRC.rglob("*.py"):
        ...
        for node in ast.walk(tree):
            ...
            elif isinstance(node, ast.Call) and isinstance(node.func, ast.Name) and node.func.id in GLOBAL_FORBIDDEN_CALLS:
        ...
        if "evaluator" in path.parts and imports & EVALUATOR_FORBIDDEN_IMPORTS:
            failures.append(f"Evaluator I/O import in {path.relative_to(ROOT)}: {sorted(imports & EVALUATOR_FORBIDDEN_IMPORTS)}")
    if failures:
        print("\n".join(failures), file=sys.stderr)
        return 1
    print("Architecture boundaries valid: Python authoritative, offline evaluator, no legacy execution authority")
    return 0
