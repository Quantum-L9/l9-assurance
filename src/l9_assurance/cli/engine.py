...
class AssuranceEngine:
    ...
    def plan(self, subject: Mapping[str, Any]) -> dict[str, Any]:
        ...
        producers = {
            item["id"]: item for item in self.configuration["producerRegistry"]["producers"]
        }
        ...
        for control in self.profile["controls"]:
            ...
            for requirement in control.get("evidenceRequirements", []):
                ...
                if check is None or producer is None:
                    raise ValueError(
                        f"Plan references unregistered evidence requirement {requirement}"
                    )
                if check["owner"] != producer["id"]:
                    raise ValueError(f"Plan check {check['id']} owner mismatch")
                producer_ids.add(producer["id"])
                check_ids.add(check["id"])
                freshness = requirement.get(
                    "freshness", control.get("freshness", {"mode": "revision-bound"})
                )
                item = {
            ...
            planned_waiver = {
                "allowed": bool(waiver.get("allowed", False)),
                "requiredRoles": list(waiver.get("requiredRoles", [])),
            }
            ...
            waiver_rules.append(
                {
                    ...
                    "allowed": bool(waiver.get("allowed", False)),
                    "requiredRoles": list(waiver.get("requiredRoles", [])),
                    "maximumDurationSeconds": waiver.get("maximumDurationSeconds"),
                }
            )
        controls_for_digest = sorted(
            self.profile["controls"], key=lambda item: (item["id"], item["version"])
        )
        preimage = {
            "schema": PLAN_SCHEMA,
            ...
            "profile": {
                ...
                "digest": sha256_digest(self.profile["profile"]),
            },
            "policy": {
                "id": self.configuration["policy"]["id"],
                "version": self.configuration["policy"]["version"],
                "digest": sha256_digest(self.configuration["policy"]),
            },
            "protocol": {
                ...
                "bundleDigest": deepcopy(self.configuration["protocolBundleDigest"]),
            },
            ...
            "sourceDigests": {
                "producerRegistry": sha256_digest(self.configuration["producerRegistry"]),
                "checkRegistry": sha256_digest(self.configuration["checkRegistry"]),
                "controls": sha256_digest(controls_for_digest),
            },
        }
        plan_digest = sha256_digest(preimage)
        plan = {
            **preimage,
            "planId": f"plan_{plan_digest['value'][:40]}",
    ...
    def admit(
        ...
        *,
        ...
        return admit_observations(
            ...
            {
                "subject": subject,
                "producerRegistry": self.configuration["producerRegistry"],
                "checkRegistry": self.configuration["checkRegistry"],
                "receivedAt": received_at or self.clock(),
                "channel": channel,
                "replayStore": self.replay_store,
            },
        )
    ...
    def evaluate(
        ...
        *,
        ...
        identity = decision_id or _derive_decision_id(
            subject, self.profile, policy, accepted_evidence, timestamp
        )
...
def verify_plan(plan: Any) -> dict[str, Any]:
    ...
    if supplied and supplied.get("algorithm") == "sha256":
        preimage = {
            ...
            if key not in {"planId", "planDigest"}
        }
    ...
    digest = sha256_digest(
        {
            ...
            "evidence": sorted(item["envelope"]["evidenceId"] for item in evidence),
