...
def run_consumer_conformance(
    *,
    ...
    try:
        ...
        transported_digest = ""
    expected_verdict = (
        canonical_decision.get("verdict", "")
        if isinstance(canonical_decision, Mapping) and verification["valid"]
        else ""
    )
    try:
        expected_summary = (
            render_decision_summary(canonical_decision) if verification["valid"] else ""
        )
    ...
    checks = [
        _check(
            "byte-transport",
            bool(canonical) and transported_decision_text == canonical,
            "decision transported byte-for-byte",
        ),
        _check(
            "digest",
            bool(canonical_digest) and transported_digest == canonical_digest,
            "decision digest preserved",
        ),
        _check(
            "verdict",
            bool(expected_verdict) and published_verdict.lower() == expected_verdict,
            "published verdict matches",
        ),
        _check(
            "summary",
            bool(expected_summary) and published_summary == expected_summary,
            "summary is canonical projection",
        ),
